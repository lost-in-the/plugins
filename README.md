...
| `woods-plugin` | Four guide skills — `woods-setup`, `woods-mcp-config`, `woods-investigate`, `woods-diagnose` — for [woods](https://github.com/lost-in-the/woods), the Rails code-intelligence gem: install/upgrade, MCP configuration, index-driven investigation (audits, code reviews, impact analysis), and diagnosis. | [`lost-in-the/woods`](https://github.com/lost-in-the/woods) → `plugin/` | woods **≥ 2.0.0** |
